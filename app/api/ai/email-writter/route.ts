...
export async function POST(request: Request) {
  ...
  if (action === "changeMood") {
    aiPrompt = `
Reescribe el siguiente correo electrónico manteniendo el mismo contenido pero ajustando el tono para que sea ${mood}:
...
El resultado debe ser un texto que mantenga la esencia del correo original pero con el nuevo tono solicitado.
    ...
    aiPrompt = `
Escribe un correo electrónico utilizando como base el siguiente texto: "${prompt}".
